...
func initRoutes(mx *mux.Router, formatter *render.Render) {
	...
	mx.HandleFunc("/Reviews", getAllReviewsHandler(formatter)).Methods("GET")
	mx.HandleFunc("/user/Reviews", submitReviewHandler(formatter)).Methods("POST")
}
...
func submitReviewHandler(formatter *render.Render) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		var result review
		if err = c.Find(bson.M{"Name": r.Name}).One(&result); err != nil {
			formatter.JSON(w, http.StatusInternalServerError, err.Error())
			return
		}
